...
export default function AddReview({ user }) {
  ...
  const editingReview =
    location.state?.currentReview || location.state?.review || null;

  const [review, setReview] = React.useState(
    editingReview?.review || editingReview?.text || "",
  );
  const [error, setError] = React.useState("");
  const [saving, setSaving] = React.useState(false);

  React.useEffect(() => {
    setReview(editingReview?.review || editingReview?.text || "");
  }, [editingReview]);
    ...
    <div className="stack-gap">
      <section className="section-hero">
        ...
        <h1 className="page-title">
          {editingReview ? "Chỉnh sửa đánh giá của bạn." : "Viết review cho bộ phim này."}
        </h1>
        <p className="page-subtitle mb-0">
          Review sẽ được gửi tới endpoint `/api/v1/movies/review` của backend
          Lab03/Lab04/Lab05/Lab06 theo đúng mô hình route-controller-dao.
        </p>
      </section>
